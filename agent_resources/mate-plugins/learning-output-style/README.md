...
**Note:** This plugin differs from the original unshipped Learning output style by also incorporating all functionality from the [explanatory-output-style plugin](https://github.com/anthropics/mate/tree/main/plugins/explanatory-output-style), providing both interactive learning and educational insights.
...
When enabled, this plugin automatically adds instructions at the start of each session that encourage Mate to:
...
Instead of implementing everything automatically, Mate will:
...
The plugin uses a SessionStart hook to inject additional context into every session. This context instructs Mate to adopt an interactive teaching approach where you actively participate in writing key parts of the code.

## When Mate requests contributions

Mate will ask you to write code for:
...
## When Mate won't request contributions

Mate will implement directly:
...
**Mate:** I've set up the authentication middleware. The session timeout behavior is a security vs. UX trade-off - should sessions auto-extend on activity, or have a hard timeout?
...
In addition to interactive learning, Mate will provide educational insights about implementation choices using this format:
...
This SessionStart hook pattern is roughly equivalent to MATE.md, but it is more flexible and allows for distribution through plugins.
...
- Update the plugin - create a local copy of this plugin to personalize it
  - Hint: Ask Mate to read https://docs.mate.com/en/docs/mate/plugins.md and set it up for you!

## Philosophy

Learning by doing is more effective than passive observation. This plugin transforms your interaction with Mate from "watch and learn" to "build and understand," ensuring you develop practical skills through hands-on coding of meaningful logic.
