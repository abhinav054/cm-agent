...
A comprehensive toolkit for developing Mate plugins with expert guidance on hooks, MCP integration, plugin structure, and marketplace publishing.

## Overview

The plugin-dev toolkit provides seven specialized skills to help you build high-quality Mate plugins:
...
4. **Plugin Settings** - Configuration patterns using .mate/plugin-name.local.md files
...
7. **Testing** - Verify plugin works in Mate
...
**Trigger phrases:** "create a hook", "add a PreToolUse hook", "validate tool use", "implement prompt-based hooks", "${MATE_PLUGIN_ROOT}", "block dangerous commands"
...
- ${MATE_PLUGIN_ROOT} for portable paths
...
- Environment variable expansion (${MATE_PLUGIN_ROOT}, user vars)
...
- ${MATE_PLUGIN_ROOT} usage throughout
...
**What it covers:**
- .mate/plugin-name.local.md pattern for configuration
...
- AI-assisted agent generation using Mate's proven prompt
...
- 3 reference docs: agent-creation-system-prompt (from Mate), system-prompt-design (~4,000w), triggering-examples (~2,500w)
...
- Based on skill-creator methodology adapted for Mate plugins
...
Install from mate-marketplace:

```bash
/plugin install plugin-dev@mate-marketplace
```
...
This keeps Mate's context focused while providing deep knowledge when needed.
...
- Based on official Mate documentation
...
✅ **Portability**
- Use ${MATE_PLUGIN_ROOT} everywhere
- Relative paths only
...
This plugin is part of the mate-marketplace. To contribute improvements:
