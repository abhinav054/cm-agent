...
Practical examples of commands designed for Mate plugins, demonstrating plugin-specific patterns and features.
...
!`node ${MATE_PLUGIN_ROOT}/scripts/quality-check.js $1`
...
**Key features:**
- Uses `${MATE_PLUGIN_ROOT}` for portable path
...
**Security scan:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/security-scan.sh $1`

**Performance analysis:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/best-practices.sh $1`
...
Template structure: @${MATE_PLUGIN_ROOT}/templates/api-documentation.md
...
!`bash ${MATE_PLUGIN_ROOT}/scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash ${MATE_PLUGIN_ROOT}/scripts/package.sh $1`
...
Deployment configuration for $1: @${MATE_PLUGIN_ROOT}/config/$1-deploy.json
...
- Plugin's linting rules: ${MATE_PLUGIN_ROOT}/config/lint-rules.json
- Security checklist: ${MATE_PLUGIN_ROOT}/checklists/security.md
- Performance guidelines: ${MATE_PLUGIN_ROOT}/docs/performance.md
...
Run plugin analyzer: !`node ${MATE_PLUGIN_ROOT}/scripts/analyze.js $1`
...
Template: @${MATE_PLUGIN_ROOT}/templates/review-report.md
...
Check build script exists: !`test -x ${MATE_PLUGIN_ROOT}/scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f ${MATE_PLUGIN_ROOT}/config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @${MATE_PLUGIN_ROOT}/config/$1.json

**Execute build:** !`bash ${MATE_PLUGIN_ROOT}/scripts/build.sh $1 2>&1`

**Validation results:** !`bash ${MATE_PLUGIN_ROOT}/scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @${MATE_PLUGIN_ROOT}/config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash ${MATE_PLUGIN_ROOT}/scripts/test-full.sh`
- Security scan: !`bash ${MATE_PLUGIN_ROOT}/scripts/security-scan.sh`
- Performance audit: !`bash ${MATE_PLUGIN_ROOT}/scripts/perf-check.sh`
- Compliance check: !`bash ${MATE_PLUGIN_ROOT}/scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash ${MATE_PLUGIN_ROOT}/scripts/test-basic.sh`
- Quick lint: !`bash ${MATE_PLUGIN_ROOT}/scripts/lint.sh`
...
```markdown
Check exists: !`test -f ${MATE_PLUGIN_ROOT}/path/file && echo "YES" || echo "NO"`
```
...
1. **Test with plugin installed:**
   ```bash
   cd /path/to/plugin
   mate /command-name args
   ```

2. **Verify ${MATE_PLUGIN_ROOT} expansion:**
   ```bash
   # Add debug output to command
   !`echo "Plugin root: ${MATE_PLUGIN_ROOT}"`
   ```

3. **Test across different working directories:**
   ```bash
   cd /tmp && mate /command-name
   cd /other/project && mate /command-name
   ```

4. **Validate resource availability:**
   ```bash
   # Check all plugin resources exist
   !`ls -la ${MATE_PLUGIN_ROOT}/scripts/`
   !`ls -la ${MATE_PLUGIN_ROOT}/config/`
   ```

### Common Mistakes to Avoid

1. **Using relative paths instead of ${MATE_PLUGIN_ROOT}:**
   ```markdown
   # Wrong
   !`node ./scripts/analyze.js`

   # Correct
   !`node ${MATE_PLUGIN_ROOT}/scripts/analyze.js`
   ```
...
2. **Forgetting to allow required tools:**
   ...
   allowed-tools: Bash(*)
   ---
   !`bash ${MATE_PLUGIN_ROOT}/scripts/script.sh`
   ```
...
4. **Hardcoding plugin paths:**
   ...
   @/home/user/.mate/plugins/my-plugin/config.json

   # Correct - works everywhere
   @${MATE_PLUGIN_ROOT}/config.json
   ```
