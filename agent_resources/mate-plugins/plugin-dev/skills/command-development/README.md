...
Comprehensive guidance on creating Mate slash commands, including file format, frontmatter options, dynamic arguments, and best practices.
...
- Plugin-specific features (${MATE_PLUGIN_ROOT}, plugin patterns)
...
- **plugin-features-reference.md**: Plugin-specific command features
  - Plugin command discovery and organization
  - ${MATE_PLUGIN_ROOT} environment variable usage
  - Plugin command patterns (configuration, template, multi-script)
...
Mate activates this skill when users:
...
Mate loads references and examples as needed based on task.
...
- **Project**: `.mate/commands/` (shared with team)
- **Personal**: `~/.mate/commands/` (your commands)
...
| `model` | Specify model | `sonnet`, ``, `haiku` |
...
- ✓ Plugin command patterns (${MATE_PLUGIN_ROOT}, discovery, organization)
...
6. Test examples work with current Mate
