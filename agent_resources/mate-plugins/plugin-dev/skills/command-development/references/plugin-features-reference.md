...
This reference covers features and patterns specific to commands bundled in Mate plugins.

## Table of Contents

- [Plugin Command Discovery](#plugin-command-discovery)
- [MATE_PLUGIN_ROOT Environment Variable](#mate_plugin_root-environment-variable)
...
Mate automatically discovers commands in plugins using the following locations:
...
## MATE_PLUGIN_ROOT Environment Variable

### Purpose

`${MATE_PLUGIN_ROOT}` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node ${MATE_PLUGIN_ROOT}/scripts/analyze.js`

Read template: @${MATE_PLUGIN_ROOT}/templates/report.md
```
...
Lint results: !`node ${MATE_PLUGIN_ROOT}/bin/lint.js $1`
...
Configuration: @${MATE_PLUGIN_ROOT}/config/deploy-config.json
...
Use this template: @${MATE_PLUGIN_ROOT}/templates/api-report.md
...
Step 1 - Prepare: !`bash ${MATE_PLUGIN_ROOT}/scripts/prepare.sh $1`
Step 2 - Config: @${MATE_PLUGIN_ROOT}/config/$1.json
Step 3 - Execute: !`${MATE_PLUGIN_ROOT}/bin/execute $1`
...
1. **Always use for plugin-internal paths:**
   ```markdown
   # Good
   @${MATE_PLUGIN_ROOT}/templates/foo.md
...
2. **Validate file existence:**
   ...
   !`test -f ${MATE_PLUGIN_ROOT}/config.json && echo "exists" || echo "missing"`

   If config exists, load it: @${MATE_PLUGIN_ROOT}/config.json
...
4. **Combine with arguments:**
   ```markdown
   Run: !`${MATE_PLUGIN_ROOT}/bin/process.sh $1 $2`
   ```
...
- Verify syntax is exact: `${MATE_PLUGIN_ROOT}`
...
Load configuration: @${MATE_PLUGIN_ROOT}/deploy-config.json
...
Template: @${MATE_PLUGIN_ROOT}/templates/component-docs.md
...
Validate: !`bash ${MATE_PLUGIN_ROOT}/scripts/validate.sh`
...
Environment config: @${MATE_PLUGIN_ROOT}/config/$1.json
...
Cache directory: ${MATE_PLUGIN_ROOT}/cache/

Analyze @$1 and save results to cache:
!`mkdir -p ${MATE_PLUGIN_ROOT}/cache && date > ${MATE_PLUGIN_ROOT}/cache/last-run.txt`
...
- Mate will automatically use Task tool to launch agent
...
- Mention skill by name to hint Mate should invoke it
...
- Config exists: !`test -f ${MATE_PLUGIN_ROOT}/config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d ${MATE_PLUGIN_ROOT}/scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x ${MATE_PLUGIN_ROOT}/bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node ${MATE_PLUGIN_ROOT}/scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use ${MATE_PLUGIN_ROOT} for all plugin-internal paths**
