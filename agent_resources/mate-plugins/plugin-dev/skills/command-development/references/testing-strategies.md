...
head -n 20 .mate/commands/my-command.md | grep -A 10 "^---"

# Check for closing frontmatter marker
head -n 20 .mate/commands/my-command.md | grep -c "^---" # Should be 2

# Verify file has .md extension
ls .mate/commands/*.md

# Check file is in correct location
test -f .mate/commands/my-command.md && echo "Found" || echo "Missing"
```
...
if echo "$FRONTMATTER" | grep -q "^model:"; then
  ...
  if ! echo "sonnet  haiku" | grep -qw "$MODEL"; then
    echo "ERROR: Invalid model '$MODEL' (must be sonnet, , or haiku)"
    exit 1
  fi
...
cat > .mate/commands/test-bash.md << 'EOF'
---
...
cat > .mate/commands/test-forbidden.md << 'EOF'
---
...
# Command: .mate/commands/risky-operation.md
...
COMMANDS_CHANGED=$(git diff --cached --name-only | grep "\.mate/commands/.*\.md")
  ...
  test:
    ...
    steps:
      ...
      - name: Validate command structure
        run: |
          for cmd in .mate/commands/*.md; do
            echo "Testing: $cmd"
      ...
      - name: Check for TODOs
        run: |
          if grep -r "TODO" .mate/commands/; then
...
```bash
# Monitor Mate during command execution
# In terminal 1:
mate --debug

# In terminal 2:
watch -n 1 'ps aux | grep mate'
...
```bash
# Check file location
ls -la .mate/commands/my-command.md

# Check permissions
chmod 644 .mate/commands/my-command.md

# Check syntax
head -n 20 .mate/commands/my-command.md
...
```bash
# Verify syntax
grep '\$1' .mate/commands/my-command.md
grep '\$ARGUMENTS' .mate/commands/my-command.md

# Test with simple command first
echo "Test: \$1 and \$2" > .mate/commands/test-args.md
```
...
```bash
# Check allowed-tools
grep "allowed-tools" .mate/commands/my-command.md

# Verify command syntax
grep '!\`' .mate/commands/my-command.md
