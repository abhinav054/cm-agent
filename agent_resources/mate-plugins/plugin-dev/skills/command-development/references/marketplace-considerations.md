...
if [ ! -f ".mate/command-initialized" ]; then
...
Checking for user config: .mate/plugin-name.local.md

if [ -f ".mate/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .mate/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .mate/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .mate/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .mate/plugin-name.local.md to customize"
fi
...
Or set in .mate/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".mate/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .mate/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .mate/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .mate/operation-completed.flag
```
