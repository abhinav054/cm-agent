...
Detailed analysis of how production plugins use the `.mate/plugin-name.local.md` pattern.
...
SWARM_STATE_FILE=".mate/multi-agent-swarm.local.md"
...
```bash
cat > "$WORKTREE_PATH/.mate/multi-agent-swarm.local.md" <<EOF
---
...
sed "s/^pr_number: .*/pr_number: $PR_NUM/" \
  ".mate/multi-agent-swarm.local.md" > temp.md
mv temp.md ".mate/multi-agent-swarm.local.md"
```
...
Document any changes needed in MATE.md.
```
...
**Purpose:** Prevent session exit and loop Mate's output back as input
...
RALPH_STATE_FILE=".mate/ralph-loop.local.md"
...
# Create state file
cat > ".mate/ralph-loop.local.md" <<EOF
---
iteration: 1
...
echo "Ralph loop initialized: .mate/ralph-loop.local.md"
```
...
|---------|-------------------|--------------|
| **File** | `.mate/multi-agent-swarm.local.md` | `.mate/ralph-loop.local.md` |
...
```bash
# BAD
FILE="/Users/alice/.mate/my-plugin.local.md"

# GOOD
FILE=".mate/my-plugin.local.md"
```
...
The `.mate/plugin-name.local.md` pattern provides:
