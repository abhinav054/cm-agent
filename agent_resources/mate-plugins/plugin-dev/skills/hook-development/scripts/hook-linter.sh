...
check_script() {
  ...
  if grep -E '^[^#]*/home/|^[^#]*/usr/|^[^#]*/opt/' "$script" | grep -q .; then
    ...
    echo "   Use \$MATE_PROJECT_DIR or \$MATE_PLUGIN_ROOT"
    ((warnings++))
  fi

  # Check 8: Uses MATE_PLUGIN_ROOT
  if ! grep -q "MATE_PLUGIN_ROOT\|MATE_PROJECT_DIR" "$script"; then
    echo "💡 Tip: Use \$MATE_PLUGIN_ROOT for plugin-relative paths"
  fi
