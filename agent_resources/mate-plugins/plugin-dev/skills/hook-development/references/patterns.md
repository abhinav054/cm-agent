...
This reference provides common, proven patterns for implementing Mate hooks. Use these patterns as starting points for typical hook use cases.
...
if [ -f "package.json" ]; then
  ...
  echo "export PROJECT_TYPE=rust" >> "$MATE_ENV_FILE"
fi
```
  ...
  "Notification": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MATE_PLUGIN_ROOT}/scripts/log-notification.sh"
        }
      ]
    }
  ]
  ...
  "PostToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MATE_PLUGIN_ROOT}/scripts/check-quality.sh"
        }
      ]
    }
  ]
...
FLAG_FILE="$MATE_PROJECT_DIR/.enable-security-scan"
...
**Note:** Must restart Mate after creating/removing flag files for hooks to recognize changes.
...
```bash
#!/bin/bash
CONFIG_FILE="$MATE_PROJECT_DIR/.mate/my-plugin.local.json"
...
**Configuration file (.mate/my-plugin.local.json):**
```json
{
  "strictMode": true,
  "maxFileSize": 500000,
