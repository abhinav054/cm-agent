  ...
  "PreToolUse": [
    {
      ...
      "hooks": [
        {
          "type": "command",
          "command": "bash ${MATE_PLUGIN_ROOT}/scripts/quick-check.sh",
          "timeout": 5
        },
        {
          "type": "prompt",
...
if [ -f ".mate-hooks-config.json" ]; then
  strict_mode=$(jq -r '.strict_mode' .mate-hooks-config.json)
...
**Example .mate-hooks-config.json:**
```json
{
  "strict_mode": true,
...
# Append to audit log
echo "$timestamp | $USER | $tool_name | $input" >> ~/.mate/audit.log
...
export MATE_PROJECT_DIR="/tmp/test-project"
export MATE_PLUGIN_ROOT="$(pwd)"
mkdir -p "$MATE_PROJECT_DIR"
