...
1. **Scan enabled plugins**: Read `.mate-plugin/plugin.json` for each
2. **Discover components**: Look in default and custom paths
3. **Parse definitions**: Read YAML frontmatter and configurations
4. **Register components**: Make available to Mate
5. **Initialize**: Start MCP servers, register hooks

**Timing**: Component registration happens during Mate initialization, not continuously.

### Activation Phase

When components are used:

**Commands**: User types slash command → Mate looks up → Executes
**Agents**: Task arrives → Mate evaluates capabilities → Selects agent
**Skills**: Task context matches description → Mate loads skill
**Hooks**: Event occurs → Mate calls matching hooks
...
**Note**: Mate doesn't support nested command discovery automatically. Use custom paths:
...
**Note**: Use build script to combine files, Mate doesn't support file references.
...
```bash
#!/bin/bash
source "${MATE_PLUGIN_ROOT}/lib/test-utils.sh"
run_tests
```
