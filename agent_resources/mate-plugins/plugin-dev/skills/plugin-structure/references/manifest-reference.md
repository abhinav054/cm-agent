...
**Required path**: `.mate-plugin/plugin.json`

The manifest MUST be in the `.mate-plugin/` directory at the plugin root. Mate will not recognize plugins without this file in the correct location.
...
```json
{
  "mcpServers": {
    "github": {
      "command": "node",
      "args": ["${MATE_PLUGIN_ROOT}/servers/github-mcp.js"],
      "env": {
...
Mate validates the manifest on plugin load:
