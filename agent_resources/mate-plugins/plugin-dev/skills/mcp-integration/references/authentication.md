...
Complete guide to authentication methods for MCP servers in Mate plugins.
...
Mate automatically handles the complete OAuth 2.0 flow for SSE and HTTP servers:

1. User attempts to use MCP tool
2. Mate detects authentication needed
3. Opens browser for OAuth consent
4. User authorizes in browser
5. Tokens stored securely by Mate
...
No additional auth configuration needed! Mate handles everything.
...
Tokens are stored securely by Mate:
...
```json
{
  "api": {
    ...
    "headersHelper": "${MATE_PLUGIN_ROOT}/scripts/get-headers.sh"
  }
}
```
...
{
  "secure-api": {
    "command": "${MATE_PLUGIN_ROOT}/servers/mtls-wrapper",
...
```json
{
  "headersHelper": "${MATE_PLUGIN_ROOT}/scripts/generate-jwt.sh"
}
```
