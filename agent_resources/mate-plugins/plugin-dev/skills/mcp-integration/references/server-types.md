...
Complete reference for all MCP server types supported in Mate plugins.
...
{
  "my-server": {
    "command": "${MATE_PLUGIN_ROOT}/servers/custom-server",
...
1. **Startup**: Mate spawns process with `command` and `args`
2. **Communication**: JSON-RPC messages via stdin/stdout
3. **Lifecycle**: Process runs for entire Mate session
4. **Shutdown**: Process terminated when Mate exits
...
{
  "custom": {
    "command": "${MATE_PLUGIN_ROOT}/servers/my-server.js",
    "args": ["--verbose"]
  }
}
```
...
1. **Use absolute paths or ${MATE_PLUGIN_ROOT}**
