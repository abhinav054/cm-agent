...
Complete guide to using MCP tools effectively in Mate plugin commands and agents.

## Overview

Once an MCP server is configured, its tools become available with the prefix `mcp__plugin_<plugin-name>_<server-name>__<tool-name>`. Use these tools in commands and agents just like built-in Mate tools.
...
- Can use any tool Mate determines is necessary
...
Mate automatically structures tool calls based on schema:

```typescript
// Mate generates this internally
{
...
```markdown
Steps:
1. Make parallel calls (Mate handles this automatically):
...
2. **Install plugin locally** in `.mate-plugin/`
3. **Verify tools available** with `/mcp`
4. **Test command** that uses tools
5. **Check debug output**: `mate --debug`
