...
Guide the user through creating a complete, high-quality Mate plugin from initial concept to tested implementation. Follow a systematic approach: understand requirements, design components, clarify details, implement following best practices, validate, and test.
...
3. Create directory structure using bash:
   ```bash
   mkdir -p plugin-name/.mate-plugin
...
6. Create .gitignore if needed (for .mate/*.local.md, etc.)
...
2. For each command:
   ...
   - Write instructions FOR Mate (not TO user)
...
2. Create .mcp.json configuration with:
   ...
   - Command and args (with ${MATE_PLUGIN_ROOT})
...
3. Create example .mate/plugin-name.local.md file (as documentation)
4. Implement settings reading in hooks/commands as needed
5. Add to .gitignore: `.mate/*.local.md`
...
**Goal**: Test that plugin works correctly in Mate
...
1. **Installation instructions**:
   - Show user how to test locally:
     ...
     ```
   - Or copy to `.mate-plugin/` for project testing
...
3. **Testing recommendations**:
   ...
   - For hooks: Use `mate --debug` to see hook execution
...
- **Apply best practices**:
  ...
  - Commands written FOR Mate
  - Strong trigger phrases
  - ${MATE_PLUGIN_ROOT} for portability
