...
def consume_stop_state(session_id):
    ...
    The Stop hook is asyncRewake — it runs in the background after Mate's
...
def capture_git_baseline(cwd):
    ...
    files are not reviewed as Mate-authored.
    """
    try:
...
# `.git/mate-trailers`) so it survives across sessions and is per-clone.
#
