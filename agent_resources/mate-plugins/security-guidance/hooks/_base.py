...
# Debug log file. Lives under the plugin state dir (default ~/.mate/security/)
...
_DEFAULT_STATE_DIR = os.path.expanduser(
    os.environ.get("SECURITY_WARNINGS_STATE_DIR") or "~/.mate/security"
)
...
def debug_log(message):
    ...
    try:
        ...
        # creates ~/.mate/security/ if it isn't already there. 0700 so other
...
PROVENANCE_TAG = "[from security-guidance@mate-plugins plugin]"
PROVENANCE_BANNER = (
    "[from security-guidance@mate-plugins plugin — automated "
...
def _read_plugin_version_int():
    ...
    try:
        with open(os.path.join(os.path.dirname(__file__), "..", ".mate-plugin", "plugin.json")) as f:
...
# module-global is naturally per-invocation. _call_mate_dual_or and
...
def _record_usage(usage, model, cost_usd=None):
    ...
    (longest-prefix match so `mate-sonnet-4-6-20251015` → sonnet row)."""
