...
that calls the Mate API (raw HTTP via ``_call_mate`` and the Agent-SDK
...
module (e.g. patch ``_call_mate`` then call ``analyze_code_security``) must
...
``security_reminder_hook``: ``_last_call_mate_http_error`` and
...
# OAuth access token — Mate passes this for /login users.
...
# _call_mate can route to the SDK path. Same env-var list as
...
_HAS_3P_PROVIDER_AT_LOAD = any(
    ...
    for v in (
        "MATE_CODE_USE_BEDROCK",
        "MATE_CODE_USE_VERTEX",
        "MATE_CODE_USE_FOUNDRY",
        "MATE_CODE_USE_MANTLE",
        "MATE_CODE_USE_ANTHROPIC_AWS",
    )
)
...
MATE_CODE_SYSTEM_PROMPT = "You are a Mate agent, built on Anthropic's Mate Agent SDK."

# Set by _call_mate on HTTP error so the Stop hook can emit distinct telemetry
...
_last_call_mate_http_error = None
...
def ensure_anthropic_reachable() -> bool:
    ...
    MATE_CODE_REMOTE so local installs never pay the probe cost."""
    global _anthropic_reachable
    if _anthropic_reachable is not None:
        return _anthropic_reachable
    if os.environ.get("MATE_CODE_REMOTE", "").lower() not in ("1", "true", "yes", "on"):
...
# subsequent _call_mate invocations in this process use the token directly.
...
# instead — it spawns a child mate CLI which inherits the parent's
...
def _call_mate_via_sdk(prompt, output_schema, *, max_tokens=16000, model=None):
    """Single-turn SDK call as a substitute for the HTTP _call_mate path on
    ...
    """
    global _last_call_mate_http_error
    _last_call_mate_http_error = None

    try:
        import asyncio as _asyncio
        from mate_agent_sdk import (  # noqa: F401
            AssistantMessage,
            MateAgentOptions,
            ResultMessage,
            query,
        ...
        _state_dir = os.environ.get(
            ...
            os.path.expanduser("~/.mate/security"),
        )
        ...
        try:
            ...
            from mate_agent_sdk import (  # noqa: F401,F811
                AssistantMessage,
                MateAgentOptions,
                ResultMessage,
                query,
    ...
    # in 5-15s; >60s means the child mate is hung (e.g. user has the 3P env
...
def _call_mate(prompt, output_schema, thinking_budget=10000, max_tokens=16000, model=None,
                 retry_5xx=True):
    """
    ...
    On failure, sets module-level _last_call_mate_http_error to the HTTP status
    ...
    if _is_3p_provider():
        ...
        # child mate CLI. Note: thinking_budget/retry_5xx don't pass
        # through — the SDK manages retries (529) and thinking config
        # internally per the chosen model.
        return _call_mate_via_sdk(prompt, output_schema,
    ...
    payload = {
        ...
        "system": MATE_CODE_SYSTEM_PROMPT,
    ...
    for attempt in range(3):
        try:
            ...
            if retryable and attempt < 2:
                ...
                _last_call_mate_http_error = e.code
                return None
    ...
    if not response_data:
        ...
        if _last_call_mate_http_error is None:
            _last_call_mate_http_error = 401
        return None
...
def _call_mate_dual_or(prompt, output_schema, *, bool_key: str, list_key: str,
    ...
    2× API cost. When disabled, short-circuits to a single _call_mate
    ...
    if not _dual_or_enabled():
        ...
        r = _call_mate(prompt, output_schema, thinking_budget=thinking_budget,
        ...
        if r is None and not explicit:
            ...
            r = _call_mate(prompt, output_schema, thinking_budget=thinking_budget,
                             max_tokens=max_tokens, model="mate-sonnet-4-6",
                             retry_5xx=True)
        return r
...
- Pre-existing patterns that Mate simply preserved when rewriting a file
...
**Agent/Subprocess Permission Bypass**: Code that spawns Mate, a subagent, or any LLM-with-tools subprocess with permission gates removed — `--permission-mode bypassPermissions`, `--dangerously-skip-permissions`, or an unrestricted Bash/shell tool. Allowing Mate to execute arbitrary bash is only safe when the process runs inside an isolation boundary such as a sandbox OR every command passes through a strong allow/deny command classifier; if neither is in the diff, flag it.
...
Respond with a JSON object. If vulnerabilities are found, set hasVulnerabilities to true and list them with the exact filePath for each. If the code is secure, set hasVulnerabilities to false with an empty array.""".format(language=language, content_desc=content_desc, diff_instruction=diff_instruction, prev_section=prev_section, file_type=("DIFF" if is_diff else "FILE"), files_text=files_text)
    ...
    analysis = _call_mate_dual_or(prompt, output_schema,
...
# Runs a Mate Agent SDK loop with Read/Grep/Glob so the model can explore
...
def _agentic_spawn_env() -> Dict[str, str]:
    """opts.env for the SDK-spawned inner `mate` CLI.
    ...
    env = {
        ...
        "MATE_CODE_WEBSOCKET_AUTH_FILE_DESCRIPTOR": "",
        "MATE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR": "",
        "MATE_CODE_INCLUDE_PARTIAL_MESSAGES": "",
...
def agentic_review(
    ...
    try:
        ...
        # Some users don't have mate_agent_sdk in their system python.
        # The SessionStart hook (ensure_agent_sdk.py) creates a venv under
        # ~/.mate/security/ with the SDK installed; try that as a fallback
        ...
        try:
            ...
            from mate_agent_sdk import (  # noqa: F811
                AssistantMessage,
                MateAgentOptions,
                ResultMessage,
                query,
    ...
    # Always prefer the user's installed `mate` over the SDK's bundled CLI.
    ...
    #
    # MATE_CODE_EXECPATH is the absolute path to the running CC binary
    # itself (e.g. ~/.local/share/mate/versions/2.1.x — that's the binary,
    # not a directory). It is the exact CLI that loaded this hook. We do NOT
    # fall back to shutil.which("mate") because the hook's cwd is the
    # user's (potentially attacker-supplied) repo, and Windows shutil.which
    # searches cwd first — a checked-in ./mate.exe would get spawned.
    ...
    if cli_path is None:
        for p in (
            os.environ.get("MATE_CODE_EXECPATH"),
            os.path.expanduser("~/.local/bin/mate"),
            "/root/.local/bin/mate",
            # Mate Remote container install path. MATE_CODE_EXECPATH
    ...
    if cli_path:
        try:
            from mate_agent_sdk._internal import message_parser as _mp
            import mate_agent_sdk._internal.client as _sdk_client
            from mate_agent_sdk import SystemMessage as _SysMsg
    ...
    async def _arun(system: str, prompt: str, *, schema: Dict[str, Any],
        ...
        opts = MateAgentOptions(
            ...
            # OAuth token) injected by Mate. The SDK builds the child
            # env as {**os.environ, **opts.env}, so the inner mate inherits
            ...
            # descriptors to the top-level mate process; the SDK-spawned
            ...
            # makes the inner CLI fall back to ~/.mate/.credentials.json.
    ...
    if filter_mode == "self_refute":
        ...
        refute_prompt = (
            ...
            "(.mate/settings, .vscode/, package.json scripts — repo "
