...
1. ``mate-security-guidance.md`` — markdown appended to every LLM review prompt.
...
Discovery, in precedence order (matching MATE.md / settings.json):
  - ``~/.mate/<name>``                  (user)
  - ``<cwd>/.mate/<name>``              (project, committed)
  - ``<cwd>/.mate/<name>.local.<ext>``  (project local, gitignored)

Managed delivery via ``managed-settings.json`` is not yet supported.
Org admins can still push files to ``~/.mate/`` via MDM/GPO.
...
GUIDANCE_BASENAME = "mate-security-guidance.md"
...
def load_for_session(cwd: Optional[str]) -> None:
    ...
    try:
        ...
        debug_log(f"extensibility: failed to load mate-security-guidance.md: {e}")
        _guidance_block = ""
    try:
...
def _config_paths(cwd: Optional[str], basename: str) -> List[Tuple[str, str]]:
    ...
    paths = [("User", os.path.expanduser(os.path.join("~", ".mate", basename)))]
    if cwd:
        paths.append(("Project", os.path.join(cwd, ".mate", basename)))
        # mate-security-guidance.local.md / security-patterns.local.yaml
        stem, ext = os.path.splitext(basename)
        paths.append(("Project (local)", os.path.join(cwd, ".mate", f"{stem}.local{ext}")))
    return paths
...
def _load_guidance(cwd: Optional[str]) -> str:
    ...
    if len(combined) > GUIDANCE_MAX_BYTES:
        debug_log(
            f"extensibility: mate-security-guidance.md combined size "
...
def _load_user_patterns(cwd: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for label, path in _config_paths(cwd, "security-patterns"):
        ...
        # ".mate/security-patterns" or ".mate/security-patterns.local");
