...
```
/plugin install security-guidance@mate-plugins-official
```

Marketplace ships enabled by default in Mate — no setup beyond having the CLI itself.
...
Drop a `mate-security-guidance.md` in any of:

- `~/.mate/mate-security-guidance.md` — user-wide rules
- `<project>/.mate/mate-security-guidance.md` — project rules, intended to be committed
- `<project>/.mate/mate-security-guidance.local.md` — local overrides, intended to be `.gitignore`'d
...
Built-in rules cover common web-vulnerability classes without it — `mate-security-guidance.md` is for things specific to your codebase that the model can't infer.

## Privacy and data handling

The plugin sends data to a model endpoint to perform its reviews. Specifically, each Stop-hook diff review transmits the changed file paths, the diff hunks, and the relevant file contents in the diff; each agentic commit review additionally transmits any files the reviewer pulls in via `Read`/`Grep`/`Glob` while tracing data flow. Your `mate-security-guidance.md` contents (user, project, and local) are appended to the prompt on every review, so don't put secrets in it.

Where that data goes depends on your Mate configuration:
...
The plugin writes its own debug log to `~/.mate/security/log.txt` (override with `SECURITY_GUIDANCE_DEBUG_LOG`). The log contains diffstate metadata and finding categories — no full file contents or model prompts — and rotates at 1 MB. Nothing is uploaded.
...
Open an issue on the [security-guidance plugin repo](https://github.com/anthropics/mate/issues) with:
- The Mate CLI version (`mate --version`)
- Provider setup (1P / Bedrock / Vertex / LLM gateway / etc.)
- A minimal repro diff
- The relevant section of `~/.mate/security/log.txt`
