...
This analyzes your request and creates `.mate/hookify.warn-rm.local.md`.

### 2. Test It Immediately

**No restart needed!** Rules take effect on the very next tool use.

Ask Mate to run a command that should trigger the rule:
```
Run rm -rf /tmp/test
```
...
- **`stop`**: Triggers when Mate wants to stop (for completion checks)
...
**This rule blocks the operation** - Mate will not be allowed to execute these commands.
...
**This rule warns but allows** - Mate sees the message but can still proceed.
...
**This blocks Mate from stopping** if no test commands appear in the session transcript. Enable only when you want strict enforcement.
...
```bash
rm .mate/hookify.my-rule.local.md
```
...
This plugin is part of the Mate Marketplace. It should be auto-discovered when the marketplace is installed.
...
**Rule not triggering:**
1. Check rule file exists in `.mate/` directory (in project root, not plugin directory)
