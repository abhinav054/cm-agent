...
```
{
  ...
  "prompt": "You are analyzing a Mate conversation to find behaviors the user wants to prevent.
...
2. Corrections or reversions (user fixing Mate's actions)
...
For each confirmed behavior, create a `.mate/hookify.{rule-name}.local.md` file:
...
{Message to show Mate when rule triggers}
```
...
**IMPORTANT**: Rule files must be created in the current working directory's `.mate/` folder, NOT the plugin directory.

Use the current working directory (where Mate was started) as the base path.

1. Check if `.mate/` directory exists in current working directory
   - If not, create it first with: `mkdir -p .mate`

2. Use Write tool to create each `.mate/hookify.{name}.local.md` file
   - Use relative path from current working directory: `.mate/hookify.{name}.local.md`
   - The path should resolve to the project's .mate directory, not the plugin's

3. Show user what was created:
   ```
   Created 3 hookify rules:
   - .mate/hookify.dangerous-rm.local.md
   - .mate/hookify.console-log.local.md
   - .mate/hookify.sensitive-files.local.md
...
4. Create `.mate/hookify.dangerous-rm.local.md`:
   ```markdown
   ---
...
- **File location**: Create files in project's `.mate/` directory (current working directory), NOT the plugin's .mate/
...
2. Ensure `.mate/` directory exists (create with mkdir if needed)
3. Use absolute path if needed: `{cwd}/.mate/hookify.{name}.local.md`
4. Verify file was created with Glob or ls

**If rule doesn't trigger after creation:**
1. Verify file is in project `.mate/` not plugin `.mate/`
